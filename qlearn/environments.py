...
class GridWorld(Environment):
    def __init__(
        ...
        super().__init__(
            [Action(0, "Down"), Action(1, "Left"), Action(2, "Up"), Action(3, "Right")],
            [int, int] if quality_definitions is None else quality_definitions,
            State((0, 0)),
        )
        self.size = size
...
class ChaseGridWorld(GridWorld):
    ...
    def update_target(self):
        ...
        if self.target_state.qualities[0] == self.size - 1 - right_spacing:
            if self.target_state.qualities[1] == self.size - 1 - top_spacing:
                self.target_state = State(
                    (self.target_state.qualities[0] - 1, self.target_state.qualities[1])
                )
            else:
                self.target_state = State(
                    (self.target_state.qualities[0], self.target_state.qualities[1] + 1)
                )
        ...
        if self.target_state.qualities[0] > self.size - 1:
            self.target_state = State(
                (self.size - right_spacing, self.target_state.qualities[1])
            )
        ...
        if self.target_state.qualities[1] > self.size - 1:
            self.target_state = State(
                (self.target_state.qualities[0], self.size - top_spacing)
            )
    ...
    def take_action(self, agent: QAgent, action) -> "tuple[State, float]":
        ...
        s = State(
            (
                s.qualities[0],
                s.qualities[1],
                int(sign(self.target_state.qualities[0] - s.qualities[0])),
                int(sign(self.target_state.qualities[1] - s.qualities[1])),
            )
        )
        return s, r
