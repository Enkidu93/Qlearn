...
class QTable:
    ...
    def update(self, s_prev, a_prev, s, r, alpha, gamma, possible_actions):
        if s_prev is not None and a_prev is not None:
            self[s_prev][a_prev] = self[s_prev][a_prev] + alpha*(r + gamma*self.max_q_for(s, possible_actions) - self[s_prev][a_prev])
...
class Environment(ABC):
    def __init__(self, possible_actions:'list[Action]', quality_definitions:'list[type]', starting_state:State) -> None:
    ...
    def take_action(self, action:Optional[Action]=None):
        if action is None:
            if random.random() < self.epsilon:
                ...
                action = self.q_table.action_with_max_q_for(self.state, self.environment.possible_actions)

        if action not in self.environment.possible_actions:
            raise ValueError(f"{action.name} with value {action.value} is not one of the possible actions in this environment. Possible actions are [{self.possible_actions}]")
